...
@Component({
  ...
  template: `
    <div class="fixed bottom-24 left-1/2 -translate-x-1/2 w-11/12 max-w-md md:bottom-8">
      <div class="bg-black/80 backdrop-blur-lg rounded-full p-2 text-white shadow-lg">
        <div class="flex justify-between items-center gap-1">
          <button 
            *ngFor="let type of types" 
            (click)="setActiveType(type)" 
            [class.bg-white/20]="activeType === type"
            [class.text-white/60]="activeType !== type"
